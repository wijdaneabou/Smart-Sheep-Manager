...
export default function ClientsScreen() {
  ...
  async function fetchClients(pageNum = 1) {
    setError(null);
  ...
  useFocusEffect(
    useCallback(() => {
      setLoading(true);
      fetchClients(1).finally(() => setLoading(false));
    }, [])
  );
  ...
  const filteredClients = useMemo(() => {
    if (filter === "ALL") return clientsList;
    return clientsList.filter((c: Client) => c.type === filter);
  }, [clientsList, filter]);

  function handleFilterChange(newFilter: ClientType | "ALL") {
    setFilter(newFilter);
    setPage(1);
  }
  ...
  function renderClientItem({ item }: { item: Client }) {
    const typeInfo = CLIENT_TYPE_CONFIG[item.type] || CLIENT_TYPE_CONFIG.ACHETEUR;

    return (
      <Pressable
        style={styles.card}
          onPress={() => router.push(`/commercial/clients/${item.id}` as any)}
      >
        ...
        <View style={styles.actionRow}>
          {canEdit && (
            <Pressable
              ...
              onPress={(e) => {
                e.stopPropagation();
                router.push(`/commercial/clients/${item.id}/edit` as any);
              }}
            >
    ...
    <SafeAreaView style={styles.safeArea} edges={["top", "bottom"]}>
      <View style={styles.container}>
        ...
        <View style={styles.filterRow}>
          ...
          {Object.entries(CLIENT_TYPE_CONFIG).map(([key, config]) => (
            <FilterPill
              ...
              label={config.label}
              active={filter === key}
              onPress={() => handleFilterChange(key as ClientType)}
            />
          ))}
        </View>
        ...
        <Pagination
          ...
          onPrev={() => fetchClients(page - 1)}
          onNext={() => fetchClients(page + 1)}
        />

        {canCreate && (
          <Link href={"/commercial/clients/create" as any} asChild>
