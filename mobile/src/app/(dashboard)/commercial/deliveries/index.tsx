...
const STATUS_CONFIG: Record<DeliveryStatus, { label: string; color: string; bgColor: string }> = {
  EN_ATTENTE: { label: "En attente", color: "#D97706", bgColor: "#FEF3C7" },
  EN_COURS: { label: "En cours", color: "#2563EB", bgColor: "#DBEAFE" },
  LIVRE: { label: "Livré", color: "#15803D", bgColor: "#DCFCE7" },
};
...
export default function DeliveriesScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      setLoading(true);
      fetchDeliveries(1).finally(() => setLoading(false));
    }, [])
  );
  ...
  const filteredDeliveries = useMemo(() => {
    if (filter === "ALL") return deliveriesList;
    return deliveriesList.filter((d: Delivery) => d.status === filter);
  }, [deliveriesList, filter]);

  function handleFilterChange(newFilter: DeliveryStatus | "ALL") {
    setFilter(newFilter);
    setPage(1);
  }
  ...
  function renderDeliveryItem({ item }: { item: Delivery }) {
    const statusInfo = STATUS_CONFIG[item.status] || STATUS_CONFIG.EN_ATTENTE;

    return (
      <Pressable
        style={styles.card}
        onPress={() => router.push(`/commercial/deliveries/${item.id}` as any)}
      >
        ...
        <View style={styles.actionRow}>
          {canEdit && (
            <Pressable
              ...
              onPress={(e) => {
                e.stopPropagation();
                router.push(`/commercial/deliveries/${item.id}/edit` as any);
              }}
            >
    ...
    <SafeAreaView style={styles.safeArea} edges={["top", "bottom"]}>
      <View style={styles.container}>
        ...
        <View style={styles.filterRow}>
          ...
          {Object.entries(STATUS_CONFIG).map(([key, config]) => (
            <FilterPill
              ...
              label={config.label}
              active={filter === key}
              onPress={() => handleFilterChange(key as DeliveryStatus)}
            />
          ))}
        </View>
        ...
        <Pagination
          ...
          onPrev={() => fetchDeliveries(page - 1)}
          onNext={() => fetchDeliveries(page + 1)}
        />

        {canCreate && (
          <Link href={"/commercial/deliveries/create" as any} asChild>
