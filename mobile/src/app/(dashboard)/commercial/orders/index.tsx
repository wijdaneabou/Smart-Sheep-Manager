...
const STATUS_CONFIG: Record<OrderStatus, { label: string; color: string; bgColor: string }> = {
  BROUILLON: { label: "Brouillon", color: "#666", bgColor: "#F3F4F6" },
  ENVOYE: { label: "Envoyé", color: "#2563EB", bgColor: "#DBEAFE" },
  VALIDE: { label: "Validé", color: "#15803D", bgColor: "#DCFCE7" },
  EN_PREPARATION: { label: "En préparation", color: "#D97706", bgColor: "#FEF3C7" },
  EXPEDIE: { label: "Expédié", color: "#7C3AED", bgColor: "#EDE9FE" },
  LIVRE: { label: "Livré", color: "#1D4ED8", bgColor: "#DBEAFE" },
  FACTURE: { label: "Facturé", color: "#DC2626", bgColor: "#FEE2E2" },
  PAYE: { label: "Payé", color: "#15803D", bgColor: "#DCFCE7" },
};
...
export default function OrdersScreen() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return ordersList.filter((o: Order) => o.status === filter);
  }, [ordersList, filter]);

  function handleFilterChange(newFilter: OrderStatus | "ALL") {
    setFilter(newFilter);
    setPage(1);
  }
  ...
  function renderOrderItem({ item }: { item: Order }) {
    const statusInfo = STATUS_CONFIG[item.status] || STATUS_CONFIG.BROUILLON;

    return (
      <Pressable
        style={styles.card}
        onPress={() => router.push(`/commercial/orders/${item.id}` as any)}
      >
        ...
        <View style={styles.priceRow}>
          ...
          <Text style={styles.priceValue}>{Number(item.total).toFixed(2)} MAD</Text>
        </View>
        ...
        <View style={styles.actionRow}>
          {canEdit && (
            <Pressable
              ...
              onPress={(e) => {
                e.stopPropagation();
                router.push(`/commercial/orders/${item.id}/edit` as any);
              }}
            >
    ...
    <SafeAreaView style={styles.safeArea} edges={["top", "bottom"]}>
      <View style={styles.container}>
        ...
        <View style={styles.filterRow}>
          ...
          {Object.entries(STATUS_CONFIG).map(([key, config]) => (
            <FilterPill
              ...
              label={config.label}
              active={filter === key}
              onPress={() => handleFilterChange(key as OrderStatus)}
            />
          ))}
        </View>
        ...
        <Pagination
          ...
          onPrev={() => fetchOrders(page - 1)}
          onNext={() => fetchOrders(page + 1)}
        />

        {canCreate && (
          <Link href={"/commercial/orders/create" as any} asChild>
